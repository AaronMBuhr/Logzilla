...
public:
    ...
    explicit ArrayQueue(int size)
        : data_(size), head_pos_(0), next_pos_(-1)
    {
        if (size <= 0) {
...
private:
    ...
    // With this design the queue is considered full when next_pos_ equals head_pos_ (and the queue is non?empty).
    bool isFullLocked() const {
        return (next_pos_ != -1) && (next_pos_ == head_pos_);
    }

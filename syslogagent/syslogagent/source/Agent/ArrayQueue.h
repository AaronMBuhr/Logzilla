...
public:
	...
	inline bool isEmpty() { return next_pos_ == -1; }
...
bool ArrayQueue<T>::enqueue(T&& item)
{
	...
	// Logger::debug2("enqueue start: head_pos_==%d  next_pos_==%d\n", head_pos_, next_pos_);
	...
	Logger::debug2("ArrayQueue::enqueue() success initial head_pos_==%d next_pos_==%d, finish head_pos==%d next_pos==%d\n", initial_head, initial_next, head_pos_, next_pos_);
	return result;
}

template<class T>
bool ArrayQueue<T>::dequeue(T& item) {
	//Logger::debug2("ArrayQueue::dequeue() start: head_pos_==%d  next_pos_==%d\n", head_pos_, next_pos_);
	int initial_head = head_pos_;
	int initial_next = next_pos_;
	if (isEmpty()) {
	...
	data_locker_.unlock();
	Logger::debug2("ArrayQueue::dequeue() success initial head_pos_==%d next_pos_==%d, finish head_pos==%d next_pos==%d\n", initial_head, initial_next, head_pos_, next_pos_);
	return true;
}
...
bool ArrayQueue<T>::removeFront(T& item) {
	//Logger::debug2("ArrayQueue::removeFront() release start: head_pos_==%d  next_pos_==%d\n", head_pos_, next_pos_);
	if (isEmpty()) {
	...
	data_locker_.unlock();
	Logger::debug2("ArrayQueue::removeFront(item) success initial head_pos_==%d next_pos_==%d, finish head_pos==%d next_pos==%d\n", initial_head, initial_next, head_pos_, next_pos_);
	return result;
}

template<class T>
bool ArrayQueue<T>::removeFront() {
	//Logger::debug2("releaseOne start: head_pos_==%d  next_pos_==%d\n", head_pos_, next_pos_);
	if (isEmpty()) {
	...
	data_locker_.unlock();
	Logger::debug2("ArrayQueue::removeFront() success initial head_pos_==%d next_pos_==%d, finish head_pos==%d next_pos==%d\n", initial_head, initial_next, head_pos_, next_pos_);
	return true;
}

template<class T>
size_t ArrayQueue<T>::length() {
	if (next_pos_ == -1) {
		return 0;
	}
	if (head_pos_ >= next_pos_) {

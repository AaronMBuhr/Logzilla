...
namespace SyslogAgent.Config
{
    public class Registry
    {
        ...
        public void ReadConfigFromRegistry( ref Configuration config )
        {
            ...
            {
                string value = mainKey.GetValue( SharedConstants.RegistryKey.OnlyWhileRunning, Convert.ToString( SharedConstants.ConfigDefaults.OnlyWhileRunning ) ).ToString();
                switch( value.ToLower() )
                {
                    ...
                    case "1":
                    case "yes":
                        config.OnlyWhileRunning = true;
                        break;

                    case "false":
                    case "0":
                    case "no":
                        config.OnlyWhileRunning = false;
                        break;

                    default:
                        config.OnlyWhileRunning = SharedConstants.ConfigDefaults.OnlyWhileRunning == 1;
                        break;
                }
            }
